...
# sectors/room number the Poodle landed on
# random.randint returns an int between the two ints
...
# PLAYER is a dictionary of images used for animation frames
PLAYER = {
...
# number of animation frame to display
player_frame = 0
# stores the current image of the astronaut
...
for key, room_scenery_list in scenery.items():
    for scenery_item_list in room_scenery_list:
        # checksum checks all data is present and correct
        # by calculating involving data
        # then checking the result against the correct answer
...
# each planet surface location is 13 tiles by 13 tiles
# room_coordinate counts from 0 to 12
for room_coordinate in range(0, 13):
    for room_number in [1, 2, 3, 4, 5]: # Add top fence
        # add a piece of scenery to scenery list for current room
        # 31 is a fence
        # [scenery, y, x]
...
# The room data is reused for each new room player goes into
# calculates room size, where the doors are, and places floors & walls
    global room_map, room_width, room_height, room_name, hazard_map
    global top_left_x, top_left_y, wall_transparency_frame

    # returns elements in list of current_room in GAME_MAP list
    room_data = GAME_MAP[current_room]
    room_name = room_data[0]
    room_height = room_data[1]
    room_width = room_data[2]

    # set floor types in current room
    ...
    if current_room % MAP_WIDTH != 1: # If room is not on left of map
        # if room is on left edge of map, there cannot be an exit
        # if remainder is 1, the room is on the left edge (rooms 26, 31, 36 etc.)
        # check if there is a room to the left of current room
        # if we need an exit
        room_to_left = GAME_MAP[current_room - 1]
        # If room on the left has a right exit, add left exit in this room

        # check if room to the left has a right exit
        # if yes, apply floor type
        if room_to_left[4]: 
    ...
    # checks for current_room in scenery dictionary  
    if current_room in scenery:
        # for each scenery item in current_room's scenery dictionary
        for this_scenery in scenery[current_room]: 
            scenery_number = this_scenery[0]
            scenery_y = this_scenery[1]
            scenery_x = this_scenery[2]
            # places scenery item at position scenery_y, scenery_x
            room_map[scenery_y][scenery_x] = scenery_number

            # need to figure out how iwde an image is
            # so we know how many tiles it fills
            # scenery_number is the dict key to get info
            # about the object from the objects dictionary
            # grab image from objects dictionary with scenery_number key
            image_here = objects[scenery_number][0]
            image_width = image_here.get_width()
            image_width_in_tiles = int(image_width / TILE_SIZE)

            # 255 represents a space that doesn't have an object in it
            # it also cannot be walked on
            for tile_number in range(1, image_width_in_tiles):
                # covers tiles to the right
    ...
    for prop_number, prop_info in props.items():
        ...
        #checks if prop is sitting on the floor
        if (prop_room == current_room and
            room_map[prop_y][prop_x] in [0, 39, 2]):
                ...
                image_width = image_here.get_width()
                image_width_in_tiles = int(image_width / TILE_SIZE)
                #block tiles with prop with extended size
                for tile_number in range(1, image_width_in_tiles):
...
## GAME LOOP ##
###############
# 1. when a player presses a movement key, the program changes the player's
# position (even if that puts them somewhere impossible aka inside a wall
# 2. the program carries out any actions the player requires using the object
# at the players location.  this means the player and the object are in the
# same position in the room at this time.
# 3. if the player is standing somewhere they're not allowed to be (such as
# inside a wall), the program moves them back to where they were.
...
def game_loop():
    ...
    if player_frame > 0:
        player_frame += 1
        time.sleep(0.05)
        # when player_frame reaches 5, reset animation
        if player_frame == 5:
            player_frame = 0
            player_offset_x = 0
            player_offset_y = 0
...
# used to reset position if player walks somewhere invalid
...
# check for exiting the room
    if player_x == room_width: # through door on RIGHT
        ...
        # change room number currently in
        current_room += 1
        # generate new room map
        generate_map()
        player_x = 0 # enter at left
        # room_height / 2 player is right in the middle of the doorway
    ...
    if keyboard.tab and len(in_my_pockets) > 0:
        selected_item += 1
        # if the selected_item cursor past the end of the list,
        # it is reset to the first item again
...
#### Teleport section ends
  ...
  # If the player is standing somewhere they shouldn't, move them back.
    ...
    # player_offset_x and player_offset_y are used to position
    # the astronaut correctly when walking into a new tiles
...
def draw():
    ...
    #draws big red rectangle
    #wipes out the previous screen display
    #Rect((left position, top position), (width, height))
    box = Rect((0, 150), (800, 600))
    #(box, color)
    #color can be a tupple of the red, green, and blue numbers
    #that make up the number
    screen.draw.filled_rect(box, RED)
    box = Rect ((0, 0), (800, top_left_y + (room_height - 1)*30))
    #clipping area - like an invisible window through which you view the screen
    #if a program draws something outside the window, it can't be seen
    #clipping area with height of room to stop player's shadow from spilling out
    #of the bottom of the game when theyy're in the front doorway
    screen.surface.set_clip(box)
    floor_type = get_floor_type()

    #Draw in layers that go on top of each other, start with floors
    ...
    for y in range(room_height): # Lay down floor tiles, then items on floor.
        for x in range(room_width):
            # look up object in object dictionary
            # followed by y and x positions
    ...
    # ensures that both floor pad and object on it are visible
    if current_room == 26:
    ...
    for y in range(room_height):
        for x in range(room_width):
            ...
            if item_here not in items_player_may_stand_on + [255]:
                ...
                # makes wall transparent if player walks behind it
                if (current_room in outdoor_rooms 
                    and y == room_height - 1
        ...
        #by drawing player in position after the floor has been drawn
        #makes sure the astronaughts leggs don't disappear
        #when walking up the screen        
        if (player_y == y):
                draw_player()

    #turns off clipping area that stopped shadows
...
## PROP INTERACTIONS ##
#######################
# finds the start position of object at player's poisition
# go left to find object number if location contains 255
...
def pick_up_object():
    ...
    if item_player_is_on in items_player_may_carry:
        ...
        sounds.pickup.play()
        # short delay of half a second to make sure the confirmation message
        # isn't overwritten if the player holds down the key too long
        time.sleep(0.5)
    else:
...
def add_object(item): # Adds item to inventory.
    ...
    # the item the player just picked up is selected
...
def drop_object(old_y, old_x):
    ...
    if room_map[old_y][old_x] in [0, 2, 39]: # places you can drop things
        # if item is dropped, update props dictionary
        # [room, y, x]
...
def examine_object():
    ...
    for prop_number, details in props.items():
        ...
        if details[0] == current_room: # if prop is in the room
            ...
            # checks for hidden item INSIDE the item being examined by player
...
Rendezvous Sector 13, outside."
        ...
        if current_room == 27:
            open_door(26)
        # opens door by removing door from room & the game
    ...
    for recipe in RECIPES:
        ...
        if (item_carrying == ingredient1
            ...
            # if the item the player is on is a prop
            # set its room number to zero, removing it from the game
            # if the item is a prop, its deleted from the room map for the current room
            if item_player_is_on in props.keys(): 
                props[item_player_is_on][0] = 0 
                room_map[player_y][player_x] = get_floor_type()
            # remove item being carried from inventory
            # add new item created to inventory
    ...
    if item_carrying in ACCESS_DICTIONARY:
        ...
        # if the player is standing in same room as door
        # with correct item to unlock door
...
def open_door(opening_door_number):
    ...
    # start door animation at frame 0
    door_frame_number = 0
...
def shut_engineering_door():
    ...
    # the player can see this door from either side depending which room
    # they are in.  door between rooms 25 and 26
...
generate_map()
# will run after the variables have been set up
