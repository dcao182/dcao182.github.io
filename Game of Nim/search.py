...
def depth_first_graph_search(problem):
    ...
    while frontier:
        ...
        if problem.goal_test(node.state):
            ...
            s = str(len(explored)) + "paths have been expanded and" + str(len(frontier)) + "paths remain in the frontier"
            file1.write(s)
            file1.close()
            return node
...
class EightPuzzle(Problem):
    ...
    def result(self, state, action):
        ...
        # neighbor is the target square to move to
        neighbor = blank + delta[action]
        # move the blank square to the target square
