...
def start_tcpprobe():
    ...
    os.system('echo "sport == 5001 || dport == 5001"  > /sys/kernel/debug/tracing/events/tcp/tcp_probe/filter')
    os.system('echo 1 > /sys/kernel/debug/tracing/events/tcp/tcp_probe/enable')
    Popen("cat /sys/kernel/debug/tracing/trace_pipe > %s/tcp_probe.txt" % args.dir, shell=True)


def stop_tcpprobe():
    os.system('echo 0 > /sys/kernel/debug/tracing/events/tcp/tcp_probe/enable')
    os.system('echo 0 > /sys/kernel/debug/tracing/events/tcp/tcp_probe/filter')
    os.system('pkill -9 cat')
...
def main():
    ...
    if args.cli:
        ...
        print("*** Running experiment -- please wait a couple minutes")
        run_topology_experiment(net)

    net.stop()
    end = time()
    os.system("pkill -9 bwm-ng")
